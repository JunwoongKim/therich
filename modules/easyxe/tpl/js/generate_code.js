...
function doDisplaySkinColorset(colorset)
{
	...
	var params = [];
	params.selected_widget = jQuery('select[name=selected_widget]').val();
	params.skin = skin;
...
function completeGenerateCodeInPage(widget_code) {
	...
	var newWidgetObj = opener.jQuery('#widget_' + parseInt(opener.idStep));

	switch(insertPosition)
	{
		case 'before':
			opener.jQuery('#widget_' + widgetID).before('<div id="tmpWidget"></div>');
			console.log(opener.jQuery('#tmpWidget'));
			opener.jQuery('#tmpWidget').replaceWith(newWidgetObj);
			break;
		case 'after':
			var widgetObj = opener.jQuery('#widget_' + widgetID);
			widgetObj.after(newWidgetObj);
			break;
	}
