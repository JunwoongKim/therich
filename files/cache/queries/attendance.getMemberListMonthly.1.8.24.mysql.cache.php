...
$query = new Query();
$query->setQueryId("getMemberListMonthly");
$query->setAction("select");
$query->setPriority("");
if(isset($args->year_month)) {
${'year_month1_argument'} = new ConditionArgument('year_month', $args->year_month, 'like_prefix');
${'year_month1_argument'}->createConditionValue();
if(!${'year_month1_argument'}->isValid()) return ${'year_month1_argument'}->getErrorMessage();
} else
${'year_month1_argument'} = NULL;if(${'year_month1_argument'} !== null) ${'year_month1_argument'}->setColumnType('varchar');
...
${'sort_index13_argument'}->ensureDefaultValue('attendance_monthly.monthly');
if(!${'sort_index13_argument'}->isValid()) return ${'sort_index13_argument'}->getErrorMessage();

$query->setColumns(array(
new SelectExpression('`attendance_monthly`.*')
));
...
new ConditionWithoutArgument('`member`.`member_srl`','`attendance_monthly`.`member_srl`',"equal")
,new ConditionWithArgument('`attendance_monthly`.`regdate`',$year_month1_argument,"like_prefix", 'and')))
,new ConditionGroup(array(
