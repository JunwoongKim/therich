...
<?php if($__Context->package->avail_remove){ ?>
	<div class="x_alert x_alert-block">
		<p><?php echo $__Context->lang->description_uninstall ?></p>
	</div>
	<div class="x_well x_clearfix">
		...
		<?php if($__Context->show_ftp_note){ ?>
			<p><?php echo $__Context->lang->ftp_form_title ?>. (<a href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispAdminConfigFtp') ?>">FTP Setup</a>)</p>
		<?php } ?>
	</div>
			...
			<?php if($__Context->need_password && !$__Context->directModuleInstall->toBool()){ ?>
				<div class="x_control-group">
					...
					<div class="x_controls">
						...
						<span class="x_help-block"><?php echo $__Context->lang->about_ftp_password ?></span>
					</div>
				</div>
			<?php } ?>
...
<?php if(!$__Context->package->avail_remove){ ?>
	<div class="x_alert x_alert-error">
		<?php if($__Context->package->deps){ ?><p><?php echo $__Context->lang->msg_dependency_package ?></p><?php } ?>
		<?php if(!$__Context->package->deps){ ?><p><?php echo $__Context->lang->msg_does_not_support_delete ?></p><?php } ?>
	</div>
	<?php if($__Context->package->deps){ ?><div class="x_well">
		<p><?php echo $__Context->lang->dependant_list ?>:</p>
		<ul>
			<?php if($__Context->package->deps&&count($__Context->package->deps))foreach($__Context->package->deps as $__Context->dep_package_srl){ ?><li><?php echo $__Context->installed[$__Context->dep_package_srl]->title ?></li><?php } ?>
		</ul>
	</div><?php } ?>
<?php } ?>
