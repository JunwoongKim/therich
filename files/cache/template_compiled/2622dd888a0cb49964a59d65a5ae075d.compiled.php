<?php if(!defined("__XE__"))exit;
$__Context->year = substr($__Context->selected_date,0,4) ?>
<?php $__Context->year_month = substr($__Context->selected_date,0,6) ?>
<?php $__Context->month = substr($__Context->selected_date,4,2) ?>
<?php $__Context->week = $__Context->Model->getWeek($__Context->selected_date) ?>
<?php $__Context->eom = date('t', mktime(0,0,0,$__Context->month,1,$__Context->year)) ?>
<form action="<?php echo getUrl('') ?>" method="get" class="adminSearch"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
<input type="hidden" name="mid" value="" />
<input type="hidden" name="module" value="<?php echo $__Context->module ?>" />
<input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
<input type="hidden" name="type" value="<?php echo $__Context->type ?>" />
<input type="hidden" name="selected_date" value="<?php echo $__Context->selected_date ?>" />
    <fieldset>
        <select name="selected_group_srl">
            <option value="0"><?php echo $__Context->lang->group ?></option>
            <?php if($__Context->group_list&&count($__Context->group_list))foreach($__Context->group_list as $__Context->key => $__Context->val){ ?>
            <option value="<?php echo $__Context->val->group_srl ?>" <?php if($__Context->selected_group_srl==$__Context->val->group_srl){ ?>selected="selected"<?php } ?>><?php echo $__Context->val->title ?></option>
            <?php } ?>
        </select>
        <select name="search_target">
		<option value="" <?php if(!$__Context->search_target){ ?>selected="selected"<?php } ?>><?php echo $__Context->lang->search_target ?></option>
		<option value="user_id" <?php if($__Context->search_target=='user_id'){ ?>selected="selected"<?php } ?>><?php echo $__Context->lang->user_id ?></option>
        <option value="email_address" <?php if($__Context->search_target=='email_address'){ ?>selected="selected"<?php } ?>><?php echo $__Context->lang->email ?></option>
		<!--option value="user_name" <?php if($__Context->search_target=='user_name'){ ?>selected="selected"<?php } ?>><?php echo $__Context->lang->user_name ?></option-->
		<option value="nick_name" <?php if($__Context->search_target=='nick_name'){ ?>selected="selected"<?php } ?>><?php echo $__Context->lang->nick_name ?></option>
        </select>
        <input type="text" name="search_keyword" value="<?php echo htmlspecialchars($__Context->search_keyword) ?>" class="inputTypeText" />
        <span class="button"><input type="submit" value="<?php echo $__Context->lang->cmd_search ?>" /></span>
        <a href="#" onclick="location.href='<?php echo getUrl('','type',$__Context->type,'act',$__Context->act,'module',$__Context->module) ?>';return false;" class="button"><span><?php echo $__Context->lang->cmd_cancel ?></span></a>
    </fieldset>
</form>
<?php if($__Context->search_keyword || $__Context->selected_group_srl!=0){ ?>
<?php if($__Context->user_data->data&&count($__Context->user_data->data))foreach($__Context->user_data->data as $__Context->val2){ ?>
<div id="attendance_list">
<table cellspacing="0" class="rowTable attendanceTable" >
	<tr>
		<th class="center">
			<?php echo $__Context->lang->email ?>
		</th>
		<th class="center">
			<?php echo $__Context->lang->nick_name ?>
		</th>
		<!--th class="center">
			<?php echo $__Context->lang->user_name ?>
		</th-->
		<th class="center">
			<?php echo $__Context->lang->admin_graph ?>
		</th>
		<th class="center">
			<?php echo $__Context->lang->getpoint ?>
		</th>
	</tr>
<tr>
	<td class="center" width="12%"> 
		<?php $__Context->user_info = $__Context->oMemberModel->getMemberInfoByMemberSrl($__Context->val2->member_srl) ?>
        <?php echo $__Context->user_info->email_address ?>
	</td>
	<td class="center" width="12%"> 
		<div class="member_<?php echo $__Context->user_info->member_srl ?>"><?php echo $__Context->user_info->nick_name ?></div>
	</td>
	<!--td class="center" width="12%"> 
		<?php echo $__Context->user_info->user_name ?>
	</td-->
	<td width="49%" style="width:300px;">
		<?php $__Context->total_info = $__Context->Model->getTotalData($__Context->user_info->member_srl) ?>
		<div class="admin_graph">
			<?php $__Context->percent=(int)($__Context->total_info->total/$__Context->total_info->total * 100) ?>
            <div class="progress" style="width:230px;">
                <div class="bar bar-success" style="width:<?php echo $__Context->percent ?>%;"></div>&nbsp;
                <?php if($__Context->percent>=50){;
echo $__Context->lang->view_total_day;
} ?>
            </div>
				<div class="num" style="left:230px;width:70px;">
				<strong><?php echo $__Context->total_info->total ?></strong>/<?php echo $__Context->total_info->total ?> <?php echo $__Context->lang->days ?>
                </div>
       		</div>
	</td>
	<td class="center">
		<?php echo $__Context->total_info->total_point ?>(<?php echo $__Context->lang->view_total_point ?>)
	</td>
</tr>
<tr>
	<td class="center" colspan="2">
			<?php echo $__Context->lang->yearly_info ?>(<?php echo $__Context->year ?>)
	</td>
	<td style="width:300px;">
		<div class="admin_graph_data">
			<?php $__Context->pad = date('t', mktime(0,0,0,02,1,$__Context->year)) ?>
			<?php if($__Context->pad == 29){ ?>
				<?php $__Context->yearly = 366 ?>
			<?php }else{ ?>
				<?php $__Context->yearly = 365 ?>
			<?php } ?>
			<?php $__Context->yearly_info = $__Context->Model->getYearlyData($__Context->year,$__Context->user_info->member_srl) ?>
			<div class="admin_graph">
				<?php $__Context->percent=(int)($__Context->yearly_info/$__Context->yearly * 100) ?>
                <div class="progress" style="width:230px;">
                    <div class="bar bar-success" style="width:<?php echo $__Context->percent ?>%;"></div><?php if($__Context->percent>=50){;
if($__Context->percent == 100){;
echo $__Context->lang->attendance_perfect;
}else{;
echo $__Context->percent ?>%<?php };
}else{ ?>&nbsp;<?php } ?>
                </div>
					<div class="num" style="left:230px;width:70px;">
					<strong><?php echo $__Context->yearly_info ?></strong>/<?php echo $__Context->yearly;
if($__Context->percent<50){ ?>(<?php echo $__Context->percent ?>%)<?php } ?>
					</div>
        		</div>
		</div>
	</td>
	<td class="center">
		<?php echo $__Context->Model->getTotalPoint($__Context->user_info->member_srl) ?>
	</td>
</tr>
<tr>
	<td class="center" colspan="2">
			<?php echo $__Context->lang->monthly_info ?>(<?php echo $__Context->year.'-'.$__Context->month ?>)
	</td>
	<td style="width:300px;">
		<?php $__Context->monthly_information = $__Context->Model->getMonthlyAttendance($__Context->user_info->member_srl, $__Context->year_month) ?>
		<div class="admin_graph">
			<?php $__Context->percent=(int)($__Context->monthly_information->monthly/$__Context->eom * 100) ?>
                <div class="progress" style="width:230px;">
                    <div class="bar bar-success" style="width:<?php echo $__Context->percent ?>%;"></div><?php if($__Context->percent>=50){;
if($__Context->percent == 100){;
echo $__Context->lang->attendance_perfect;
}else{;
echo $__Context->percent ?>%<?php };
}else{ ?>&nbsp;<?php } ?>
                </div>
				<div class="num" style="left:230px;width:70px;">
				<strong><?php echo $__Context->monthly_information->monthly ?></strong>/<?php echo $__Context->eom;
if($__Context->percent<50){ ?>(<?php echo $__Context->percent ?>%)<?php } ?>
				</div>
        	</div>
	</td>
	<td class="center">
		<?php echo $__Context->monthly_information->monthly_point ?>
	</td>
</tr>
<tr>
	<td class="center" colspan="2">
			<?php echo $__Context->lang->weekly_info ?>(<?php echo substr($__Context->week->monday,0,4).'-'.substr($__Context->week->monday,4,2).'-'.substr($__Context->week->monday,6,2) ?> ~ <?php echo substr($__Context->week->sunday,0,4).'-'.substr($__Context->week->sunday,4,2).'-'.substr($__Context->week->sunday,6,2) ?>)
	</td>
	<td style="width:300px;">
		<?php $__Context->weekly_information = $__Context->Model->getWeeklyData($__Context->user_info->member_srl, $__Context->week) ?>
		<div class="admin_graph">
			<?php $__Context->percent=(int)($__Context->weekly_information->weekly/7 * 100) ?>
                <div class="progress" style="width:230px;">
                    <div class="bar bar-success" style="width:<?php echo $__Context->percent ?>%;"></div><?php if($__Context->percent>=50){;
if($__Context->percent == 100){;
echo $__Context->lang->attendance_perfect;
}else{;
echo $__Context->percent ?>%<?php };
}else{ ?>&nbsp;<?php } ?>
                </div>
				<div class="num" style="left:230px;width:70px;">
				<strong><?php echo $__Context->weekly_information->weekly ?></strong>/7<?php if($__Context->percent<50){ ?>(<?php echo $__Context->percent ?>%)<?php } ?>
				</div>
        	</div>
	</td>
	<td class="center">
		<?php echo $__Context->weekly_information->weekly_point ?>
	</td>
</tr>
<tr>
	<td class="center" colspan="2">
		<?php echo $__Context->lang->attend_attended_time ?> : <?php echo substr($__Context->total_info->regdate,0,4).'-'.substr($__Context->total_info->regdate,4,2).'-'.substr($__Context->total_info->regdate,6,2) ?> <?php echo substr($__Context->total_info->regdate,8,2) ?>:<?php echo substr($__Context->total_info->regdate,10,2) ?>
	</td>
	<td class="center">
		<?php echo $__Context->lang->attendance_continuity_day ?> : <?php echo $__Context->total_info->continuity ?>
	</td>
	<td class="center">
		<?php echo $__Context->lang->attendance_continuity_point ?> : <?php echo $__Context->total_info->continuity_point ?>
	</td>
</tr>
</table>
</div>
<?php } ?>
<div class="pagination a1">
   <a href="<?php echo getUrl('page','','module_srl','') ?>" class="prevEnd"><?php echo $__Context->lang->first_page ?></a> 
    <?php while($__Context->page_no = $__Context->user_data->page_navigation->getNextPage()){ ?>
        <?php if($__Context->user_data->page == $__Context->page_no){ ?>
            <strong><?php echo $__Context->page_no ?></strong> 
        <?php }else{ ?>
            <a href="<?php echo getUrl('page',$__Context->page_no,'module_srl','') ?>"><?php echo $__Context->page_no ?></a> 
        <?php } ?>
    <?php } ?>
    <a href="<?php echo getUrl('page',$__Context->user_data->page_navigation->last_page,'module_srl','') ?>" class="nextEnd"><?php echo $__Context->lang->last_page ?></a>
</div>
<?php }else{ ?>
<table cellspacing="0" class="rowTable" >
		<td class="center" height="100px">
			<?php echo $__Context->lang->messages ?>
		</td>
</table>
<?php } ?>
