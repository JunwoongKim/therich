...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/board_skin(planner_title)','header.html') ?>
<!-- 게시물 작성 폼 -->
<form action="./" method="post" onsubmit="return procFilter(this, insert)" id="fo_write"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
...
<div class="boardWrite">
    ...
    <?php } ?>
    <?php if($__Context->evar_date){ ?>
    <script language="javascript">
      var evar_date = '<?php echo $__Context->evar_date ?>';
      jQuery('input.date').val(evar_date);
      jQuery('input.date').prev('input[type="hidden"]').val(evar_date.replace(/-/g,""));
    </script>
    <?php } ?>
